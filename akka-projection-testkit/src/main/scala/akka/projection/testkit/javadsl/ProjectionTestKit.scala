...
final class ProjectionTestKit private[akka] (testKit: ActorTestKit) {
   ...
   */
  def run[R](projection: Projection[_], assertFunction: Creator[R]): R =
   ...
   * @param assertFunction - a function that exercise the test assertions
   */
  def run[R](projection: Projection[_], max: Duration, assertFunction: Creator[R]): R =
   ...
   * @param assertFunction - a function that exercise the test assertions
   */
  def run[R](projection: Projection[_], max: Duration, interval: Duration, assertFunction: Creator[R]): R =
    runInternal(projection, assertFunction, max, interval)

  private def runInternal[R](
      projection: Projection[_],
      assertFunction: Creator[R],
      max: Duration,
      interval: Duration): R = {
    ...
    try {
      probe.awaitAssert(max, interval, () => assertFunction.create())
    } finally {
   ...
   */
  def runWithTestSink[R](projection: Projection[_], assertFunction: Function[TestSubscriber.Probe[Done], R]): R = {
